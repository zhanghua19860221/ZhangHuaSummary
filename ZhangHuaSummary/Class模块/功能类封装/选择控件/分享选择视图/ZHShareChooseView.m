...
-(void)changeChooseBlock{
    [UIView animateWithDuration:0.5 animations:^{
        z_chooseView.frame = CGRectMake(0, HEIGHT-150, WIDTH, 150);
...
-(void)initView{
    ...
    z_chooseView = [[UIView alloc] initWithFrame:CGRectMake(0, HEIGHT, WIDTH, 150)];
    [self addSubview:z_chooseView];
    z_chooseView.backgroundColor = COLORFromRGB(0xffffff);
    
    //分享类型icon选择 数组
    NSArray *shareArray = @[@"QQ",@"微信",@"通讯录"];
    //分享类型label选择 数组
    NSArray *labelArray = @[@"QQ",@"微信",@"通讯录"];
    ...
    for (int i = 0; i<shareArray.count ; i++) {
        ...
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(z_chooseView.mas_centerY).offset(-20);
            if (tempBtn) {
            ...
            make.width.mas_equalTo(WIDTH/shareArray.count);
        ...
        [label mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(z_chooseView.mas_centerY).offset(25);
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        z_chooseView.frame = CGRectMake(0, HEIGHT, WIDTH, 150);
        self.alpha = 0.0;
